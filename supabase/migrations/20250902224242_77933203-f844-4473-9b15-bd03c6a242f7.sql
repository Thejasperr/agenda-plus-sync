-- Add user_id columns to associate data with specific users
...
ALTER TABLE public.estoque ADD COLUMN user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE;
ALTER TABLE public.servicos ADD COLUMN user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE;
ALTER TABLE public.transacoes ADD COLUMN user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE public.agendamentos ALTER COLUMN user_id SET DEFAULT auth.uid();
ALTER TABLE public.estoque ALTER COLUMN user_id SET DEFAULT auth.uid();
ALTER TABLE public.servicos ALTER COLUMN user_id SET DEFAULT auth.uid();
