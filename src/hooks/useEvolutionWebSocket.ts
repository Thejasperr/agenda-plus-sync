...
export function useEvolutionWebSocket(options: UseEvolutionWebSocketOptions) {
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    if (retriesRef.current >= MAX_RETRIES) {
      console.log(`WebSocket: max retries (${MAX_RETRIES}) reached, stopping`);
      return;
    }
    const delay = Math.min(BASE_DELAY * Math.pow(2, retriesRef.current), MAX_RECONNECT_DELAY);
    const jitter = delay * 0.3 * Math.random();
    retriesRef.current += 1;
    console.log(`WebSocket: reconnect in ${Math.round((delay + jitter) / 1000)}s (${retriesRef.current}/${MAX_RETRIES})`);
    reconnectTimerRef.current = setTimeout(() => connectRef.current(), delay + jitter);
  }, []);
